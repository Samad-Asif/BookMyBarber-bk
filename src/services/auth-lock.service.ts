...
export async function checkAccountLocked(email: string): Promise<void> {
  ...
  if (data?.locked_until && new Date(data.locked_until) > new Date()) {
    throw new ApiError(403, "Account is locked for 24 hours due to too many attempts", "ACCOUNT_LOCKED", { lockedUntil: data.locked_until });
  }
...
export async function trackOtpSend(email: string): Promise<void> {
  ...
  const windowStart = data?.otp_window_start ? new Date(data.otp_window_start).getTime() : Date.now();
...
export async function lockAccount(email: string): Promise<void> {
  ...
  throw new ApiError(403, "Account is locked for 24 hours due to too many attempts", "ACCOUNT_LOCKED", { lockedUntil });
}
...
// ponytail: admin unlock endpoint — TODO
