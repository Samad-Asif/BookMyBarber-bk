...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await uploadImage(req.file.buffer, req.file.mimetype, publicId);
    ...
    if (currentProfile?.avatar_url) {
      const oldPublicId = extractCloudinaryPublicId(currentProfile.avatar_url);
      if (oldPublicId) {
        try {
          await deleteImage(oldPublicId);
        } catch {
          // ponytail: old image delete is best-effort
        }
      }
    }
    ...
    const { data: updated, error } = await supabase
      .from("profiles")
      .update({ avatar_url: result.secureUrl, updated_at: new Date().toISOString() })
      .eq("id", userId)
      .select("avatar_url")
      .single();

    if (error) {
      throw new ApiError(500, "Failed to update profile avatar", "UPDATE_FAILED");
    }
